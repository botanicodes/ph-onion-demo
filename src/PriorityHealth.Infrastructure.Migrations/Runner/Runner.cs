...
namespace PriorityHealth.Infrastructure.Migrations.Runner
{
    public class Runner
    {
        ...
        private string GetConnectionString(string connStringOrName)
        {
            if(connStringOrName.Contains(";"))
                return connStringOrName;

            ConnectionStringSettings connString = ConfigurationManager.ConnectionStrings[connStringOrName];
